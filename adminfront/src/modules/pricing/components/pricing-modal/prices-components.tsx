...
export const EditableCell: FC<EditableCellProps> = ({
	...
	const save = async () => {
		try {
			...
			handleSave({
				...record,
				pricesFormatEdit: {
					...record.pricesFormatEdit,
					[dataIndex[1]]: values.pricesFormatEdit[dataIndex[1]],
				},
			});
		} catch (errInfo) {
			messageApi.open({
	...
	useEffect(() => {
		if (dataIndex) {
			const price = record?.pricesFormatEdit[dataIndex[1]];
			form.setFieldsValue({
				pricesFormatEdit: {
					[dataIndex[1]]: record?.pricesFormatEdit[dataIndex[1]],
				},
			});
		}
